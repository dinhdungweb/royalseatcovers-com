...
// Fix iOS Safari: m-cart-drawer click handler falsely triggers close()
// when tapping inner buttons due to iOS touch event target mismatch
(function patchMCartDrawerIOS() {
  var proto = customElements.get('m-cart-drawer');
  if (!proto) return;

  var originalConnected = proto.prototype.connectedCallback;

  proto.prototype.connectedCallback = function () {
    originalConnected.call(this);

    // Override the click listener bằng cách capture touchend trước
    // để ngăn iOS misfire close() khi tap vào nút bên trong drawer
    this.addEventListener('touchend', function (e) {
      var inner = this.querySelector('.m-cart-drawer__inner');
      var closeIcon = this.querySelector('.m-cart-drawer__close');

      // Chỉ close khi tap đúng vào overlay (ngoài inner) hoặc close icon
      var isOutside = !e.target.closest('.m-cart-drawer__inner');
      var isCloseIcon = e.target === closeIcon || e.target.closest('.m-cart-drawer__close') === closeIcon;

      if (isOutside || isCloseIcon) {
        e.preventDefault();
        this.close();
      }
      // Nếu tap vào bên trong → không làm gì, để event bubble bình thường
    }.bind(this), { passive: false, capture: true });
  };
})();
